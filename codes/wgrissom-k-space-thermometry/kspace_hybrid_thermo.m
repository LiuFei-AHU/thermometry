...
dnorm = median(sqrt(acqp.dcf(:)).*abs(acqp.data(abs(acqp.data) ~= 0)))*sqrt(length(acqp.data(:)));
...
if islogical(acqp.k)
    ...
    if size(acqp.k,1) == 1 % we have undersampling only in column dim
        ...
        for ii = 1:Nc
            ...
            dataShiftHybrid = fftshift(ifft(fftshift(dataShiftHybrid,1),[],1),1)*length(acqp.k); % ifft in row dim
            dataShiftHybrid = fftshift(dataShiftHybrid,2);
            acqp.data(:,ii) = col(dataShiftHybrid(:,kShift));
            clear dataShiftHybrid;
        end
        dnorm = dnorm * sqrt(length(acqp.k));
        % fftshift the mask
        acqp.k = kShift;
        clear kShift
    end
...
% force negativity
theta(acqp.thetaSign*theta <= 0) = 0;
...
if isempty(algp.masknz) % only run if we don't already have a heating mask
    ...
    costOld = Inf;
    cost = cost_eval(acqp.data,G,f,Ac,theta,R,algp.lam,acqp.dcf,acqp.thetaSign);
    itr = 0;
    ...
    while costOld-cost >= algp.stopFrac*costOld
        ...
        % update temp shift
        if ~algp.modeltest
            theta = theta_update(acqp.data,Ac,theta,f,G,1,algp,algp.lam,R,[],acqp.dcf,algp.modeltest,acqp.thetaSign);
            
            if algp.dofigs;figure(201);
                tmp = zeros(size(acqp.mask));tmp(acqp.mask) = acqp.thetaSign*real(theta);
                subplot(121); imagesc(tmp); axis image; title 'Estimated phase';
                tmp = zeros(size(acqp.mask));tmp(acqp.mask) = acqp.thetaSign*real(theta) > 0;
...
if ~isempty(algp.masknz)
    ...
    costOld = Inf;
    cost = cost_eval(acqp.data,G,f,Ac,theta,R,[0 0],acqp.dcf,acqp.thetaSign);
    itr = 0;
    ...
    while costOld-cost >= algp.stopFrac*costOld
        ...
        % update temp shift
        theta = theta_update(acqp.data,Ac,theta,f,G,algp.masknz,algp,[0 0],R,algp.sumMask,acqp.dcf,algp.modeltest,acqp.thetaSign);
...
tmp = zeros(size(acqp.mask)); tmp(acqp.mask) = gather(theta);theta = tmp;
tmp = zeros([size(acqp.mask) Nc]);
tmp(repmat(acqp.mask,[ones(1,length(size(acqp.mask))) Nc])) = gather(f); f = tmp;
tmp = zeros(size(acqp.mask)); tmp(acqp.mask) = gather(Ac);Ac = tmp;
...
% Evaluate cost
%
function cost = cost_eval(data,G,f,Ac,theta,R,lam,dcf,thetaSign)
...
if exist('lam','var')
    if ~isempty(lam)
        cost = cost + thetaSign*lam(1)*sum(real(theta));
        if lam(2) > 0
...
%
function theta = theta_update(data,Ac,theta,f,G,masknz,algp,lam,R,sumMask,dcf,modeltest,thetaSign)
...
for nn = 1:algp.thiters
    gold = g;
    g = masknz.*gradcalc_theta(data,Ac,theta,f,G,R,lam,dcf,thetaSign);
    ...
    if ~modeltest
      dir(thetaSign*real(dir) < 0 & thetaSign*real(theta(:,end)) < thresh) = 1i*imag(dir(thetaSign*real(dir) < 0 & thetaSign*real(theta(:,end)) < thresh));
      dir(imag(dir) < 0 & imag(theta(:,end)) < thresh) = real(dir(imag(dir) < 0 & imag(theta(:,end)) < thresh));
    end

    [t,breakOut] = stepcalc_theta(dir,data,Ac,theta,f,G,R,lam,100*min(1,pi/2/max(abs(dir))),algp,g,dcf,thetaSign);

    if ~modeltest
      z = theta + t*dir;
      if any(real(z) > 0) || any(imag(z) < 0)
          %dir = z.*(z < 0) - theta;
          dir = real(z).*(thetaSign*real(z) > 0) - real(theta(:,end)) + 1i*(imag(z).*(imag(z) > 0) - imag(theta(:,end)));
          [t,breakOut] = stepcalc_theta(dir,data,Ac,theta,f,G,R,lam,1,algp,g,dcf,thetaSign);
      end
    end
...
%
function g = gradcalc_theta(data,Ac,theta,f,G,R,lam,dcf,thetaSign)
...
if lam(2) <= 0
    g = real(g) + thetaSign * lam(1); % l1 penalty derivatives; real theta
else
    g = real(g) + thetaSign * lam(1) + 1i*(imag(g) + lam(2)); % l1 penalty derivatives;
    % complex theta
end
...
%
function [t,breakOut] = stepcalc_theta(dir,data,Ac,theta,f,G,R,lam,tmax,algp,thetagrad,dcf,thetaSign)

% use boyd's backtracking line search, which usually requires fewer cost evaluations

% calculate current cost
cost = cost_eval(data,G,f,Ac,theta,R,lam,dcf,thetaSign);
...
while (costt > cost + a*t*real(thetagrad'*dir)) && t > 10^-6
    ...
    costt = cost_eval(data,G,f,Ac,thetat,R,lam,dcf,thetaSign);
