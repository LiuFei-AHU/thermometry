...
multiCoil = false;
signPatternType = 'grad'; % switch between 'convCAIPI', 'grad', 'RF' and 'chaotic'
...
if ~simulatedData
    ...
    N = 128;Nx = N;Ny = N;
    yPos = [0 0 0 0 0]; % zeros(nSlices,1); % hot spot positions
    xPos = [0 0 0 0 0];
    sigma = [20 20 20 10 10];%20*ones(nSlices,1); % stdev of hot spots
    % 5: 1/16 FOV, 10: 1/8 FOV etc
    ...
    switch heating
        case 'all' % heating on all slices 
            for ii = 1:nSlices
                hotSpot(:,:,ii) = double(~modelTest)*(-3*pi/4)*exp(-((x-xPos(ii)).^2 + (y-yPos(ii)).^2)./sigma(ii)^2); % 18 degree heat
                hotSpot(:,:,ii) = hotSpot(:,:,ii).*(img(:,:,ii) > 0); % to mask out temp outside volume for error calc
            end
        case 'single' % heating only on first slice to test for leakage
            hotSpot(:,:,1) = double(~modelTest)*(-3*pi/4)*exp(-((x-xPos(1)).^2 + (y-yPos(1)).^2)./sigma(1)^2); % 18 degree heat
            hotSpot(:,:,1) = hotSpot(:,:,1).*(img(:,:,1) > 0); % to mask out temp outside volume for error calc
    end
    ...
    % compress here instead of above
    ...
    case 'grad'
        switch nSlices
            ...
            case 4
                ...
                kWts = repmat(encodes,[floor(N/size(encodes,1)) 1]);
            case 5
                encodes = [1 1 1 1 1; 1 (1+1i)/sqrt(2) 1i (-1+1i)/sqrt(2) -1; 1 (-1+1i)/sqrt(2) -1i (1+1i)/sqrt(2) -1; 1 (1+sqrt(3)*1i)/2 (-1+1i*sqrt(3))/2 -1 (-1-1i*sqrt(3))/2];
                kWts = repmat(encodes,[floor(N/size(encodes,1)) 1]);
        end
...
if ~modelTest
    switch algorithm
        ...
        case 'complexDifference'
            algp.lam = 10^-4; % different for complex difference due to different units
    end
...
if ~modelTest
    ...
    switch algorithm
        ...
        case 'complexDifference'
            [thetakacc,~,~,~,Ac] = kspace_hybrid_thermo_inccaipi_cd(acqp,thetainit,algp);
    end
else
    cgIters = 10;
    for ii = 1:nNoise
        ...
        switch algorithm
            case 'phaseDifference'
                [thetakacc(:,:,:,ii),~,~,~,Ac] = kspace_hybrid_thermo_inccaipi(acqp,thetainit,algp);
            case 'complexDifference'
                [thetakacc(:,:,:,ii),~,~,~,Ac] = kspace_hybrid_thermo_inccaipi_cd(acqp,thetainit,algp);
        end
...
if showFigures && ~modelTest
    ...
    for ii = 1:nSlices
        subplot(3,nSlices,ii); 
        imagesc(ct*hotSpot(:,:,ii),[minDisplayTemp maxDisplayTemp]); axis image;axis off
        ...
        subplot(3,nSlices,2*nSlices+ii);
        imagesc(abs(ct*hotSpot(:,:,ii)-tempkacc(:,:,ii)),[minDisplayTemp 1]); axis image;axis off
        h = colorbar; ylabel(h,'degrees C');
