...
hsmask = false(128);
hsmask(80:81,59:69) = true;
hsmask = hsmask(63:102,36:75);

% load the one slice data
oneSlice = load('results_proc_data_1sl');
oneSlice.tempSMS_cd = oneSlice.tempSMS_cd(63:102,36:75,:,:);

% load the two slice data
twoSlice_perm2_L15 = load('results_proc_data_2sl');
twoSlice_perm2_L15.tempSMS_cd = twoSlice_perm2_L15.tempSMS_cd(63:102,36:75,:,:);

% load the three slice data
threeSlice_perm2_2enc = load('results_proc_data_3sl');
threeSlice_perm2_2enc.tempSMS_cd = threeSlice_perm2_2enc.tempSMS_cd(63:102,36:75,:,:);

% get an object mask to find boundaries of all phantom slices
N = 40;
objMask = sum(squeeze(oneSlice.tempSMS_cd) ~= 0,3) ~= 0 | ...
    sum(squeeze(twoSlice_perm2_L15.tempSMS_cd(:,:,1,:)) ~= 0,3) ~= 0 | ...
    sum(squeeze(threeSlice_perm2_2enc.tempSMS_cd(:,:,2,:)) ~= 0,3) ~= 0 | ...
    sum(squeeze(twoSlice_perm2_L15.tempSMS_cd(:,:,2,:)) ~= 0,3) ~= 0 | ...
    sum(squeeze(threeSlice_perm2_2enc.tempSMS_cd(:,:,3,:)) ~= 0,3) ~= 0 | ...
    sum(squeeze(threeSlice_perm2_2enc.tempSMS_cd(:,:,1,:)) ~= 0,3) ~= 0;
load objMask % edited one
...
for ii = 9
    ...
    img(N+1:2*N,1:2*N) = reshape(twoSlice_perm2_L15.tempSMS_cd(:,:,:,ii),[N 2*N]);
    %img(2*N+1:3*N,1:2*N) = reshape(twoSlice_perm2_R15.tempSMS_cd(:,:,:,ii),[N 2*N]);
    %img(3*N+1:4*N,1:2*N) = reshape(twoSlice_perm1_L15.tempSMS_cd(:,:,:,ii),[N 2*N]);
    %img(4*N+1:5*N,1:2*N) = reshape(twoSlice_perm1_R15.tempSMS_cd(:,:,:,ii),[N 2*N]);
    imagesc(img,[0 6]);axis image;colorbar;
    title(num2str(t(ii)));
    drawnow;
    %pause;
end
...
[x,y] = meshgrid(linspace(-2,2,120),linspace(-1,1,120));

img = zeros(3*N,3*N);
ti = 9;
img(N+1:2*N,1:N) = oneSlice.tempSMS_cd(:,:,1,ti)';
img(N+1:3*N,N+1:2*N) = reshape(twoSlice_perm2_L15.tempSMS_cd(:,:,:,ti),[N 2*N])';
img(1:3*N,2*N+1:3*N) = reshape(threeSlice_perm2_2enc.tempSMS_cd(:,:,:,ti),[N 3*N])';
figure;
imagesc(x(:),y(:),flipud(img),[0 6]);axis image;colorbar;
axis off
drawnow;

load imgSepForDisplay
imgSepForDisplay = reshape(imgSepForDisplay(63:102,36:75,:),[N 3*N]).';
...
for kk = 1:3 % loop over color channels
  threeColor(:,:,kk) = reshape(interp1(linspace(0,6,64),map(:,kk),img(:)),[3*N 3*N]);
  tmp = threeColor(:,:,kk);
  tmp2 = imgSepForDisplay(:,:,kk);
  tmp(img < 0.1) = tmp2(img < 0.1);
  threeColor(:,:,kk) = tmp;
end
% load brainMask_3sl_perm2_2enc_128
% brainMask = reshape(brainMask(63:102,36:75,:),[N 3*N]).';
...
for ii = 1:21
    ...
    imgSepForDisplay(1:40,1:40,:) = 0;
    imgSepForDisplay(81:end,1:40,:) = 0;

    % zero out third slice for 2-slice scan
    imgSepForDisplay(1:40,41:80,:) = 0;
...
v = VideoWriter('phantomSagittalTemp.mp4','MPEG-4');
v.FrameRate = 5;
v.Quality = 95;
open(v)
writeVideo(v,F)
...
for ii = 1:Nt
    ...
    tmp = twoSlice_perm2_L15.tempSMS_cd(:,:,1,ii);
...
meanTemp_3sl_2nd = [];
for ii = 1:Nt
    tmp = twoSlice_perm2_L15.tempSMS_cd(:,:,2,ii);
...
% subplot(324)
% hold on
% plot(t,meanTemp_2sl_2nd);
% plot(t,meanTemp_3sl_2nd);
